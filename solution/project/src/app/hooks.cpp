	...
	void *rcx, bool bShadowDepth)
{
	if (tf_globals::in_CBaseWorldView_DrawExecute)
	{
		...
		misc->paintSitckyMesh();
		//misc->paintViewVelocity();
...
MAKE_HOOK(
	...
	mem::findVirtual(i::pred, 15).get(), // << 9
	bool,
	CPrediction *rcx)
{
...
MAKE_HOOK(
	...
	mem::findVirtual(i::mat_sys, 79).get(),
	ITexture *,
	...
	C_BaseEntity *rcx, const Vector &vecAbsVelocity)
{
	if (reinterpret_cast<uintptr_t>(_ReturnAddress()) == s::C_BaseEntity_SetAbsVelocity_C_BasePlayer_PostDataUpdate_call.get())
	{
		if (rcx && rcx->GetClassId() == class_ids::CTFPlayer)
		{
			...
			{
				if ((pl->m_fFlags() & FL_ONGROUND) || pl->m_hGroundEntity()) {
					CALL_ORIGINAL(rcx, { vecAbsVelocity.x, vecAbsVelocity.y, vecAbsVelocity.z });
					return;
				}

				else
				{
					if (player_data->getNumRecords(pl->cast<C_TFPlayer>()) >= 1)
					{
